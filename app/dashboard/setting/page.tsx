...
export default function SettingPage() {
  ...
  const fetchApData =async ()=>{
   ...
   await supabase.auth.getSession().then(async({ error, data }: any) => {
      if (data?.session?.user?.id) {
        // setIsLoading(true);
        setUserId(data?.session?.user?.id);
       await getUserInfo(data?.session?.user?.id, setFormState);
      await  getDocuments(data?.session?.user?.id, setDocuments);
      setIsLoading(false)
      }
    });
  }
...

{
  isLoading===true?<>(
    <div className="w-screen h-screen flex justify-center items-center">
      <SettingLoading /> {/* Render the loader component */}
    </div>
  )</>
   :<> 
   <>
